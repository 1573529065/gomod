...
func (dao *AccountDAO) ZeroAwardTransWithExtend(userID uint64, extend string) (err error) {
	if _, exists := dao.ORM.Get("binlog_format"); !exists {
		dao.ORM.Exec("SET SESSION binlog_format = 'ROW'")
		dao.ORM.InstantSet("binlog_format", "ROW")
	}
	tx := dao.ORM.Begin()
	var result Account
...
// 正数视为结算，负数视为下注
func (dao *AccountDAO) IncrUserCredit(userID uint64, amount int64, extend string) (origin int64, after int64, err error) {
	if _, exists := dao.ORM.Get("binlog_format"); !exists {
		dao.ORM.Exec("SET SESSION binlog_format = 'ROW'")
		dao.ORM.InstantSet("binlog_format", "ROW")
	}
	tx := dao.ORM.Begin()
	origin, after, err = dao.IncrUserCreditInTransaction(tx, userID, amount, extend)
	if err == nil {
		tx.Commit()
	}
	return
...
//通过参数指定是下注还是结算
func (dao *AccountDAO) IncrUserCreditWithType(userID uint64, amount int64, isBet bool, extend string) (origin int64, after int64, err error) {
	if _, exists := dao.ORM.Get("binlog_format"); !exists {
		dao.ORM.Exec("SET SESSION binlog_format = 'ROW'")
		dao.ORM.InstantSet("binlog_format", "ROW")
	}
	tx := dao.ORM.Begin()
	origin, after, err = dao.IncrUserCreditInTransWithType(tx, userID, amount, isBet, extend)
	if err == nil {
		tx.Commit()
	}
	return
...
//将用户余额减少到0
func (dao *AccountDAO) DecrToZero(userID uint64, extend string, title string) (int64, error) {
	tx := dao.ORM.Begin()
	var account Account
	err := tx.Set("gorm:query_option", "FOR UPDATE").First(&account, userID).Error
	if err != nil {
...
func (dao *AccountDAO) IncrUserCreditInTransWithType(tx *gorm.DB, userID uint64,
	amount int64, isBet bool, extend string) (origin int64, after int64, err error) {
	var account Account
	//使用悲观锁
	err = tx.Set("gorm:query_option", "FOR UPDATE").First(&account, userID).Error
	if err != nil {
...
func (dao *AccountDAO) IncrUserCreditInTransaction(tx *gorm.DB, userID uint64, amount int64, extend string) (origin int64, after int64, err error) {
	var isBet bool
	if amount < 0 {
		isBet = true
	}
	return dao.IncrUserCreditInTransWithType(tx, userID, amount, isBet, extend)
}
...
func (dao *AccountDAO) LoadBlackList(userID uint64, status int32) []Account {
	result := make([]Account, 0, 10000)
	if err := dao.ORM.Where("id = ? and status=?", userID, status).Find(&result).Error; err != nil {
		log.Error("fail to load black, err: %v", err)
	}
	return result
}
...
func (dao *AccountDAO) IsSingleUser(userID uint64) bool {
	account, _ := GetUser(dao.ORM, userID)
	if account != nil && account.Chn != "" {
		//再去查对应的channel_admin
		result := make([]ChannelAdmin, 0, 100)
		if err := dao.ORM.Where("track_name=?", account.Chn).Find(&result).Error; err != nil {
			log.Error("fail to get single user, err: %v", err)
			panic(err)
		}
		if len(result) > 0 {
			if result[0].ChannelType == 5 {
				return true
			} else {
				return false
			}
